...
const user = () => {
  ...
  const getData = async ()=>{
    try {
      let res = await axios.get(API_URL+`/posting/profile/user/${router.asPath.split('?')[1]}`)
      console.log(res)
      setDataUser(res.data)
      setPostingUser(res.data.posting)
    } catch (error) {
      console.log(error)
    }
}
...
const getDataLike = async ()=>{
  try {
    let res = await axios.get(API_URL+`/like/user/${router.asPath.split('?')[1]}`)
          ...
          <div className='container mx-auto px-96 pt-5 py-20'>
              <div className='grid grid-cols-2'>
                        <div className='flex justify-center'>
                          <img src={dataUser.avatar?API_URL + dataUser.avatar:'https://cdn-icons-png.flaticon.com/512/149/149071.png'} className='h-[150px] w-[150px] rounded-full'/>
                        </div>
                        <div className='h-10 w-[613px]'>
                        <p className='text-[32px] font-extralight text-gray-600'>{dataUser.username}</p>
